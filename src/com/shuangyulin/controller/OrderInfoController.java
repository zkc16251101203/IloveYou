...
public class OrderInfoController extends BaseController {
	...
	public void list(@ModelAttribute("scenicObj") Scenic scenicObj,String orderDate,@ModelAttribute("userObj") UserInfo userObj,String shState,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (orderDate == null) orderDate = "";
		if (shState == null) shState = "";
		if(rows != 0)orderInfoService.setRows(rows);
		List<OrderInfo> orderInfoList = orderInfoService.queryOrderInfo(scenicObj, orderDate, userObj, shState, page);
	    /*计算总的页数和总的记录数*/
	    orderInfoService.queryTotalPageAndRecordNumber(scenicObj, orderDate, userObj, shState);
	    /*获取到总的页码数目*/
	...
	public String frontlist(@ModelAttribute("scenicObj") Scenic scenicObj,String orderDate,@ModelAttribute("userObj") UserInfo userObj,String shState,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (orderDate == null) orderDate = "";
		if (shState == null) shState = "";
		List<OrderInfo> orderInfoList = orderInfoService.queryOrderInfo(scenicObj, orderDate, userObj, shState, currentPage);
	    /*计算总的页数和总的记录数*/
	    orderInfoService.queryTotalPageAndRecordNumber(scenicObj, orderDate, userObj, shState);
	    /*获取到总的页码数目*/
	    int totalPage = orderInfoService.getTotalPage();
	    ...
	    request.setAttribute("shState", shState);
	    List<Scenic> scenicList = scenicService.queryAllScenic();
	    request.setAttribute("scenicList", scenicList);
	    List<UserInfo> userInfoList = userInfoService.queryAllUserInfo();
	    request.setAttribute("userInfoList", userInfoList);
	...
	public void OutToExcel(@ModelAttribute("scenicObj") Scenic scenicObj,String orderDate,@ModelAttribute("userObj") UserInfo userObj,String shState, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(orderDate == null) orderDate = "";
        if(shState == null) shState = "";
        List<OrderInfo> orderInfoList = orderInfoService.queryOrderInfo(scenicObj,orderDate,userObj,shState);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "OrderInfo信息记录"; 
        String[] headers = { "订单id","预定景点","预定日期","预定价格","预定用户","下单时间","审核状态"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<orderInfoList.size();i++) {
        	OrderInfo orderInfo = orderInfoList.get(i); 
        	dataset.add(new String[]{orderInfo.getOrderId() + "",orderInfo.getScenicObj().getScenicName(),orderInfo.getOrderDate(),orderInfo.getPrice() + "",orderInfo.getUserObj().getName(),orderInfo.getOrderTime(),orderInfo.getShState()});
        }
        /*
