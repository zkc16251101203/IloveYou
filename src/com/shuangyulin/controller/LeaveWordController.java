...
import com.shuangyulin.service.LeaveWordService;
...
public class LeaveWordController extends BaseController {
	...
	public void list(String title,String addTime,@ModelAttribute("userObj") UserInfo userObj,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (title == null) title = "";
		if (addTime == null) addTime = "";
		if(rows != 0)leaveWordService.setRows(rows);
		List<LeaveWord> leaveWordList = leaveWordService.queryLeaveWord(title, addTime, userObj, page);
	    /*计算总的页数和总的记录数*/
	    leaveWordService.queryTotalPageAndRecordNumber(title, addTime, userObj);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String title,String addTime,@ModelAttribute("userObj") UserInfo userObj,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (title == null) title = "";
		if (addTime == null) addTime = "";
		List<LeaveWord> leaveWordList = leaveWordService.queryLeaveWord(title, addTime, userObj, currentPage);
	    /*计算总的页数和总的记录数*/
	    leaveWordService.queryTotalPageAndRecordNumber(title, addTime, userObj);
	    /*获取到总的页码数目*/
	    int totalPage = leaveWordService.getTotalPage();
	...
	public void OutToExcel(String title,String addTime,@ModelAttribute("userObj") UserInfo userObj, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(title == null) title = "";
        if(addTime == null) addTime = "";
        List<LeaveWord> leaveWordList = leaveWordService.queryLeaveWord(title,addTime,userObj);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "LeaveWord信息记录"; 
        String[] headers = { "留言id","标题","留言时间","留言人","回复时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<leaveWordList.size();i++) {
        	LeaveWord leaveWord = leaveWordList.get(i); 
        	dataset.add(new String[]{leaveWord.getLeaveWordId() + "",leaveWord.getTitle(),leaveWord.getAddTime(),leaveWord.getUserObj().getName(),leaveWord.getReplyTime()});
        }
        /*
