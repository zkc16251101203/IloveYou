...
public class CommentController extends BaseController {
	...
	@InitBinder("scenicObj")
	public void initBinderscenicObj(WebDataBinder binder) {
		binder.setFieldDefaultPrefix("scenicObj.");
	}
	@InitBinder("userObj")
	...
	public void list(@ModelAttribute("scenicObj") Scenic scenicObj,String commentContent,@ModelAttribute("userObj") UserInfo userObj,String commentTime,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (commentContent == null) commentContent = "";
		if (commentTime == null) commentTime = "";
		if(rows != 0)commentService.setRows(rows);
		List<Comment> commentList = commentService.queryComment(scenicObj, commentContent, userObj, commentTime, page);
	    /*计算总的页数和总的记录数*/
	    commentService.queryTotalPageAndRecordNumber(scenicObj, commentContent, userObj, commentTime);
	    /*获取到总的页码数目*/
	...
	public void listAll(HttpServletResponse response) throws Exception {
		...
		for(Comment comment:commentList) {
			...
			jsonComment.accumulate("commentContent", comment.getCommentContent());
			jsonArray.put(jsonComment);
		}
	...
	public String frontlist(@ModelAttribute("scenicObj") Scenic scenicObj,String commentContent,@ModelAttribute("userObj") UserInfo userObj,String commentTime,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (commentContent == null) commentContent = "";
		if (commentTime == null) commentTime = "";
		List<Comment> commentList = commentService.queryComment(scenicObj, commentContent, userObj, commentTime, currentPage);
	    /*计算总的页数和总的记录数*/
	    commentService.queryTotalPageAndRecordNumber(scenicObj, commentContent, userObj, commentTime);
	    /*获取到总的页码数目*/
	    int totalPage = commentService.getTotalPage();
	    ...
	    request.setAttribute("scenicObj", scenicObj);
	...
	public void OutToExcel(@ModelAttribute("scenicObj") Scenic scenicObj,String commentContent,@ModelAttribute("userObj") UserInfo userObj,String commentTime, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(commentContent == null) commentContent = "";
        if(commentTime == null) commentTime = "";
        List<Comment> commentList = commentService.queryComment(scenicObj,commentContent,userObj,commentTime);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Comment信息记录"; 
        String[] headers = { "评论id","被评景点","评论内容","评论人","评论时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<commentList.size();i++) {
        	Comment comment = commentList.get(i); 
        	dataset.add(new String[]{comment.getCommentId() + "",comment.getScenicObj().getScenicName(),comment.getCommentContent(),comment.getUserObj().getName(),comment.getCommentTime()});
        }
        /*
