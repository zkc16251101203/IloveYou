...
function initCommentManageTool() {
	comment_manage_tool = {
		...
		search: function() {
			...
			queryParams["commentContent"] = $("#commentContent").val();
				...
				$.ajax({
					...
					success : function (comment, response, status) {
						...
						if (comment) { 
							...
							$("#comment_scenicObj_scenicId_edit").combobox({
								url:"Scenic/listAll",
							    valueField:"scenicId",
							    textField:"scenicName",
							    panelHeight: "auto",
						        ...
						        onLoadSuccess: function () { //数据加载完毕事件
									$("#comment_scenicObj_scenicId_edit").combobox("select", comment.scenicObjPri);
									//var data = $("#comment_scenicObj_scenicId_edit").combobox("getData"); 
						            //if (data.length > 0) {
						                //$("#comment_scenicObj_scenicId_edit").combobox("select", data[0].scenicId);
						            //}
								}
							});
							$("#comment_commentContent_edit").val(comment.commentContent);
							$("#comment_commentContent_edit").validatebox({
								required : true,
