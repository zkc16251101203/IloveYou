$(function () {
    $("#comment_commentTime").datebox({
        required : true,
        showSeconds: true,
        editable: false
    });
	$("#comment_scenicObj_scenicId").combobox({
	    url:'Scenic/listAll',
	    valueField: "scenicId",
        ...
        onLoadSuccess: function () { //数据加载完毕事件
            var data = $("#comment_scenicObj_scenicId").combobox("getData"); 
            if (data.length > 0) {
                $("#comment_scenicObj_scenicId").combobox("select", data[0].scenicId);
            }
        }
	});
	$("#comment_commentContent").validatebox({
		required : true, 
